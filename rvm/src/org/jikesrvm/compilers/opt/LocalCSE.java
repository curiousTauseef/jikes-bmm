...
public class LocalCSE extends CompilerPhase {
  ...
  /**
   * BMM: Is a given instruction a load or store?  
   */
  public static boolean isJavaLoadOrStore(Instruction s) {
    return isJavaLoad(s) || isJavaStore(s);
  }    
  
  /**
   * BMM: Is a given instruction a load?  
   */
  public static boolean isJavaLoad(Instruction s) {
    return GetField.conforms(s) || GetStatic.conforms(s) || ALoad.conforms(s);
  }    

  /**
   * BMM: Is a given instruction a store?  
   */
  public static boolean isJavaStore(Instruction s) {
    return PutField.conforms(s) || PutStatic.conforms(s) || AStore.conforms(s);
  }    
  ...
  protected static final class AvExCache {
     ...
     *
     * @param s the load/store instruction
     */
    public void eliminate(Instruction s) {
      ...
      if (doMemory) {
        ...
        case VM.JMM:
          eliminate_doMemory_JMM(s); break;
        case VM.BMM:
          eliminate_doMemory_BMM(s); break;
        case VM.SC:
          eliminate_doMemory_SC(s); break;
        default:
          throw new OptimizingCompilerException("Unsupported memory model: " + VM.MemoryModel);
        }
      }
    }
     ...
     * @param s
     */
    private void eliminate_doMemory_JMM(Instruction s){
      ...
      if (LocalCSE.isStoreInstruction(s) || (options.READS_KILL && LocalCSE.isLoadInstruction(s))) {
        killCachedLoadStore(s, true);
      }
    }

    /**
     * BMM: SL reordering is allowed
     * @param s
     */
    private void eliminate_doMemory_BMM(Instruction s) {
      if (LocalCSE.isJavaLoad(s)) { // load kills every pre load/store
        killCachedLoadStore(s, false);
      } else if (LocalCSE.isJavaStore(s) || (options.READS_KILL && LocalCSE.isLoadInstruction(s))) { // store kills aliased pre load/store
        killCachedLoadStore(s, true);
      }
    }
    
    /**
     * BMM: load/store on SC kill every previous load/store, a.k.a., no reordering is allowed
     * @param s
     */
    private void eliminate_doMemory_SC(Instruction s) {
      if (LocalCSE.isJavaLoadOrStore(s)) {
        killCachedLoadStore(s, false);
      }
    }
    
    private void killCachedLoadStore(Instruction s, boolean killAliasedOnly) {
      int i = 0;
