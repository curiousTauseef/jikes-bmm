...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants, SizeConstants {
  ...
  protected final void emit_iastore() {
    ...
    fence_after_store_if_necessary();
  }
   ...
   * BMM: 
   */
  private void fence_after_store_if_necessary() {
    switch (VM.MemoryModel) {
    case VM.JMM:
    case VM.BMM:
      break; // JMM and BMM don't need to fence a store
    case VM.SC:
      // currently no difference between naive and advanced impl of SC
      if (VM.MemoryModel_NaiveImpl)
        asm.emitMFENCE();
      else
        asm.emitMFENCE();
      break;
